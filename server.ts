...
function loadEnvVar(name: string): string | undefined {
  ...
  // Fallback: read .env files directly if dotenv didn't populate the var.
...
function getFirebaseAdmin() {
  if (!firebaseAdminApp) {
    ...
    // CRITICAL: Always prioritize the project ID from the service account certificate.
    // The credentials in the certificate are only valid for the project they were issued for.
    const projectId = cert.project_id || firebaseConfig.projectId;
    
    if (firebaseConfig.projectId && cert.project_id && firebaseConfig.projectId !== cert.project_id) {
      console.warn(`[Firebase] Project ID mismatch! Config: ${firebaseConfig.projectId}, Cert: ${cert.project_id}. Using Cert project ID to avoid PERMISSION_DENIED.`);
    }
...
function getFirestoreInstance() {
  ...
  console.log(`[Firebase] Getting Firestore instance for Project: ${projectId}, Database: ${dbId || "(default)"}`);
  
  // Use the specific database ID from config if available, but handle "(default)" or placeholders
...
async function checkFirestoreHealth() {
  try {
    ...
    // Simple query to verify connectivity and existence of database
    ...
    if (err.code === 5 || err.message.includes("NOT_FOUND")) {
      console.error("[Firebase] CRITICAL: The Firestore database or project was not found. If you recently remixed this app, please run the 'Firebase Setup' tool in the Settings menu to provision a new backend.");
    } else if (err.code === 7 || err.message.includes("PERMISSION_DENIED")) {
      console.error("[Firebase] CRITICAL: Firestore permission denied. This often happens if the project ID in your config doesn't match your service account. Please run the 'Firebase Setup' tool to fix this.");
    }
  }
}
...
async function startServer() {
  ...
  app.post("/api/auth/student-login", async (req, res) => {
    ...
    try {
      ...
      // Robust normalization: trim, lowercase, and collapse multiple spaces
      const normalizedName = studentName.trim().toLowerCase().replace(/\s+/g, ' ');

      // Helper to get student from a specific DB instance
      const findStudent = async (dbInstance: admin.firestore.Firestore, dbNameLabel: string) => {
        ...
        // 1. Try normalizedName search
        console.log(`[Auth][${dbNameLabel}] Querying Firestore: classrooms/${classroomId}/students where normalizedName == "${normalizedName}"`);
        ...
        // 2. Fallback: Search all students if not found via normalizedName
        console.log(`[Auth][${dbNameLabel}] Student not found via normalizedName. Fetching all students in classroom to check manually.`);
        ...
        console.log(`[Auth][${dbNameLabel}] Classroom has ${allStudents.size} students. Checking for name match...`);
        const foundDoc = allStudents.docs.find(doc => {
          ...
          if (dbName === normalizedName || dbNormalized === normalizedName) {
            return true;
          }
          return false;
        });
        ...
        if (foundDoc) {
          ...
          // Update with normalizedName for future efficiency
          await foundDoc.ref.update({ normalizedName });
          return foundDoc;
        }

        console.log(`[Auth][${dbNameLabel}] Student "${studentName}" (normalized: "${normalizedName}") not found in the ${allStudents.size} students available.`);
        return null;
      };
      ...
      try {
        ...
        // Log as warning if we have a fallback, otherwise error
        if (isNotFound && firebaseConfig.firestoreDatabaseId && firebaseConfig.firestoreDatabaseId !== "(default)") {
          console.warn(`[Auth] Primary database "${firebaseConfig.firestoreDatabaseId}" not found. This is common in remixed apps. Retrying with default database...`);
          try {
            ...
            throw new Error(`Firestore database not found. We tried the configured database ("${firebaseConfig.firestoreDatabaseId}") and the default database, but both failed. If you recently remixed this app, please run the 'Firebase Setup' tool in the Settings menu.`);
          }
        } else {
          console.error(`[Auth] Firestore query error (code: ${err.code}):`, err.message);
          if (isNotFound) {
            throw new Error(`Firestore database or path not found. Please ensure Firestore is enabled in your project and you have run the 'Firebase Setup' tool. Details: ${err.message}`);
          } else if (err.code === 7 || (err.message && err.message.includes("PERMISSION_DENIED"))) {
            throw new Error(`Firestore permission denied. Please ensure your service account has the 'Cloud Datastore User' role in the Google Cloud Console. Details: ${err.message}`);
          } else {
            throw err;
          }
        }
